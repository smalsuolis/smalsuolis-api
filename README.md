...
Get auth API_KEY. Connect to database `jdbc:postgresql://localhost:5112/smalsuolis`, you will see two databases here `auth` and `smalsuolis`, go to `auth` database, `apps` table, and copy `api_key` from the Admin app (should be first).

Copy `.env.example` to `.env` and add `AUTH_API_KEY=` value.
